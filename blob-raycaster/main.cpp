...
namespace constants
{
    ...
    // Camera frame
    const glm::vec3 Eye(0, 0.25, 2.0f);
...
{
    return (1 - (d * d) / (constants::R * constants::R)) * (1 - (d * d) / (constants::R * constants::R)) * (1 - (d * d) / (constants::R * constants::R));
}

float blinn(float d)
{
    return std::exp(-constants::R * d * d);
}

float blob(const glm::vec3 &p)
{
    return wyvill(glm::length(p)) + blinn(glm::length(p - glm::vec3(0, 1.8, 0)));
}

glm::vec3 normal(const glm::vec3 &p)
{
    const float eps = 0.001f;
    float x = blob(p) - blob(p + glm::vec3(eps, 0, 0));
    float y = blob(p) - blob(p + glm::vec3(0, eps, 0));
    float z = blob(p) - blob(p + glm::vec3(0, 0, eps));
